---
title: "ESM 244 Lab 3: Working with spatial data"
...
ggplot(data = sesb_counts_sf) +
  geom_sf(aes(fill = n_records), color = "grey90", size = 0.1) +
  scale_fill_gradientn(colors = c("lightgray","orange","purple")) +
  theme_minimal() +
...
figure out which county has most records filter by just that county take subset of other location information - watershed or location plot the entire map or just plot the county with the counts, and add the subset of point locations maybe just use spatial join dataset

```{r}
# Go to sesb_counts_sf or do another way 

# Find county with max
county_max <- sesb_counts_sf %>% 
  filter(n_records == max(n_records)) %>% 
  pull(county) 
# Or slice_max(n_records)
# Or which.max(n_records) --> This gives the row index number 

# Subset your county max
solano_sesb_sf <- sesb_ca_sf %>% 
  filter(county == county_max) 
# If you had 2 max counties it would be weird. To address the two counties you would use %in% instead of == 

# grab your geometries
...
tmap_mode(mode = "view") # this tells tmap to turn on the interactive viewer when you run this 

tm_shape(ca_counties_sf) + 
  tm_fill("land_km2", palette = "BuGn") + # Color counties according to land area
  tm_shape(sesbania_sf) + # tmap automatically converges coordinate reference system
  tm_dots()
